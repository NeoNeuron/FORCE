...
input = np.array([0])
target = np.array([0])
for i_strength in np.random.rand(n_test)*(I_range[1]-I_range[0])+I_range[0]:
# for i_strength in (0.5,1.0):
	i_duration= 4*np.pi/i_strength * 100
	i_delay= 20*np.pi/i_strength * 100
	...
	target = np.append(target, np.sin(np.arange(int(i_delay/dt))*dt*i_strength/100))
...
np.savez(f'generator-task_{n_targets:d}_test_result.npz', ft = ft_cpu, zts = zt.cpu())
...
plt.savefig(f'FORCE_Type_A_Generator_Task_{n_targets:d}_Testing.png')