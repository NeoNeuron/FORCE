...
np.savez(f'memory-task_{n_targets:d}_qvalue_result.npz', q=np.array(q_values))
# %%
# import numpy as np
# import matplotlib.pyplot as plt
# from scipy.interpolate import interp1d
# data_package = np.load(f'memory-task_10_qvalue_result.npz')
# q_values=data_package['q']
# fname = f'memory-task_10_net_hyper_pm.npz'
# params = np.load(fname)
input_flag = np.nonzero(np.diff(params['I'])>0)[0]
...
plt.tight_layout()
# fig1.savefig(f'FORCE_Type_A_Memory_Task_10_qvalue_small_step.png')
# fig1.savefig(f'FORCE_Type_A_Memory_Task_300_qvalue.png')
fig1.savefig(f'FORCE_Type_A_Memory_Task_{n_targets:d}_qvalue_old.png')
...
# fig2.savefig(f'FORCE_Type_A_Memory_Task_{n_targets:d}_qvalue_trace.png')
# %%
